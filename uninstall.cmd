...
if "%NO_PATH%"=="0" (
  ...
  powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $bin=$env:SQLIBLIND_ENV_BIN; $current=[Environment]::GetEnvironmentVariable('Path','User'); $items=@(); if($current){ foreach($item in $current.Split(';')){ if($item -and $item.TrimEnd('\') -ine $bin.TrimEnd('\')){ $items += $item } } }; [Environment]::SetEnvironmentVariable('Path',($items -join ';'),'User'); [Environment]::SetEnvironmentVariable('IMR_SQLIBLIND_HOME',$null,'User'); [Environment]::SetEnvironmentVariable('SQLIBLIND_PYTHON',$null,'User'); [Environment]::SetEnvironmentVariable('SQLIBLIND_BIN',$null,'User')" || exit /b 1
)
